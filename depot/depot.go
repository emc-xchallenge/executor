...
func (c *client) newRunContainerWorker(guid string, logger lager.Logger) func() {
	return func() {
		...
		logger.Debug("looking-up-in-store")
		container, err := c.containerStore.Get(logger, guid)
		if err != nil {
			logger.Error("failed-looking-up-in-store", err)
			return
		}
		logger.Debug("succeeded-looking-up-in-store")
		...
		startTime := time.Now()
		container, err = c.containerStore.Create(logger, container)
		if err != nil {
		...
		err = c.containerStore.Run(logger, container)
		if err != nil {
			logger.Error("failed-running-container-in-garden", err)
			c.containerStore.Fail(logger, guid, err.Error())
		}
...
func (c *client) ListContainers(tags executor.Tags) ([]executor.Container, error) {
	return c.containerStore.List(c.logger, tags)
}
...
func (c *client) GetBulkMetrics(tags executor.Tags) (map[string]executor.Metrics, error) {
	...
	c.metricsWorkPool.Submit(func() {
		containers, err := c.containerStore.List(logger, tags)
		if err != nil {
		...
		cmetrics, err := c.containerStore.Metrics(logger, containerGuids)
		if err != nil {
